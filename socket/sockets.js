...
module.exports = function (io) {
    ...
    io.on('connection', function (socket) {
        ...
        socket.on("message", async data => {
            ...
            db.Chatroom.findByIdAndUpdate(chatRoomID, { $push: { messages: chatMessage._id } })
            
            // .then(response => 
            //     console.log(`*** IF THERE IS A RESPONSE FROM CHATROOM PUSH QUERY THEN IT'S ${response}`));
            ...
            console.log(`new message is being sent to ${roomid} from ${author}`)
